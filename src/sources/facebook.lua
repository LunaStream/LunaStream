...
function Facebook:setup() return self end

function Facebook:search(query) return self:buildError("Search not supported for Facebook", "fault", "Facebook Source") end
...
function Facebook:encodeGraphQLRequest(videoId)
  ...
  local encoded = {}
  for key, value in pairs(params) do table.insert(encoded, self:urlEncode(key) .. "=" .. self:urlEncode(value)) end
...
function Facebook:urlEncode(str)
  ...
  return (str:gsub("([^%w%-%.%_%~ ])", function(c) return string.format("%%%02X", string.byte(c)) end):gsub(" ", "+"))
end

function Facebook:isLinkMatch(link) return link:match("facebook%.com") ~= nil or link:match("fb%.watch") ~= nil end
...
function Facebook:fetchVideoData(videoId, timeout)
  ...
  if not data or not data.data or not data.data.video then return nil, "Invalid response from Facebook API" end
  ...
  if videoInfo.is_live_streaming then isStream = true end
  return {
    videoUrl = videoUrl,
...
function Facebook:loadForm(query)
  ...
  if not videoId then return self:buildError(err or "Invalid Facebook URL", "fault", "Facebook Source") end

  local videoData, err = self:fetchVideoData(videoId)
  if not videoData then return self:buildError(err or "Video not available", "fault", "Facebook Source") end
...
function Facebook:loadStream(track)
  if track.info.isStream then return self:buildError("Live streams are not supported", "fault", "Facebook Source") end

  local videoData, err = self:fetchVideoData(track.info.identifier)
  if not videoData then return self:buildError(err or "Video not found", "fault", "Facebook Source") end
